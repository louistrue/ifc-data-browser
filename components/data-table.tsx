...
export function DataTable({ tableName, data, onBack, customTitle, description }: DataTableProps) {
  ...
  const [pageSize, setPageSize] = useState(100) // Large default for IFC data
              ...
              <TableHeader className="sticky top-0 bg-background z-10">
                <TableRow>
                  {columns.map((column) => (
                    <TableHead
                      key={column}
                      className="cursor-pointer hover:bg-muted/50 transition-colors whitespace-nowrap px-4 py-3"
              ...
              <TableBody>
                {paginatedData.map((row, index) => (
                  <TableRow key={startIndex + index} className="hover:bg-muted/50">
                    {columns.map((column) => (
                      <TableCell key={column} className="font-mono text-sm whitespace-nowrap px-4 py-3">
